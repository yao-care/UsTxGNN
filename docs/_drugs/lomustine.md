...
Because original indication and regulatory history are undocumented in this jurisdiction (market status: Not marketed, 0 licenses), the primary uncertainty is not mechanistic plausibility but **local regulatory and safety documentation** — this is captured as a Blocking-severity data gap (DG001) below.
...
Currently no marketing authorization is on file for this jurisdiction (Market status: **Not marketed / Not Marketed**; total licenses: **0**).

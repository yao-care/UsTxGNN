...
> ⚠️ **Market Status Note:** The dataset returns `market_status: "Not marketed"` with zero licenses for Ceftriaxone. This is almost certainly a data pipeline gap — Ceftriaxone is widely available as a generic product in the United States under multiple FDA approvals. Manual verification via the FDA Orange Book is required before drawing any regulatory conclusions from this field.
